...
CalendarService_advance_args = function(args) {
  this.calendar = null;
  this.date = null;
  this.n = null;
  if (args) {
...
CalendarService_advance_result = function(args) {
  this.success = null;
  if (args) {
...
CalendarServiceClient = exports.Client = function(output, pClass) {
    this.output = output;
    this.pClass = pClass;
    this._seqid = 0;
    this._reqs = {};
...
CalendarServiceProcessor = exports.Processor = function(handler) {
  ...
  if (this._handler.advance.length === 3) {
    Q.fcall(this._handler.advance, args.calendar, args.date, args.n)
      .then(function(result) {
        var result = new CalendarService_advance_result({success: result});
    ...
    this._handler.advance(args.calendar, args.date, args.n, function (err, result) {
      if (err == null) {
        var result = new CalendarService_advance_result((err != null ? err : {success: result}));
        output.writeMessageBegin("advance", Thrift.MessageType.REPLY, seqid);
      } else {
        var result = new Thrift.TApplicationException(Thrift.TApplicationExceptionType.UNKNOWN, err.message);
        output.writeMessageBegin("advance", Thrift.MessageType.EXCEPTION, seqid);
